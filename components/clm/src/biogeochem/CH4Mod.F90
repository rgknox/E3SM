...
#include "shr_assert.h"
  ...
  use clm_varctl         , only : iulog, use_cn, use_nitrif_denitrif, use_lch4, use_fates
  ...
  type, public :: ch4_type
     ...
     ! This is set in CanopyFluxesMod, and is also set for FATES equivalent patches
...
contains
  ...
  subroutine CH4 (bounds, &
       ...
       carbonstate_vars, carbonflux_vars, nitrogenflux_vars, ch4_vars, lnd2atm_vars, elm_fates)
    !
    ! !DESCRIPTION:
    ...
    associate(                                                                 & 
      ...
      if (nlevdecomp == 1) then
         ...
         call p2c (bounds, nlevgrnd, &
            ...
            ! RGK: This should be unnecessary. filter_soilc "should" be
            ! only on active columns
            do j=1, nlevsoi
               do fc = 1, num_soilc
                  c = filter_soilc(fc)
...
!              
      ...
      do sat = 0, 1 ! 0 == unsaturated; 1 = saturated
         ...
         call ch4_prod (bounds, &
              ...
              carbonflux_vars, nitrogenflux_vars, ch4_vars, elm_fates)
         ...
         call ch4_aere (bounds, &
              ...
              waterstate_vars, waterflux_vars, carbonstate_vars, carbonflux_vars, ch4_vars, elm_fates)
...
!  subroutine LgtWgtO2ModelForNitDenit()

    ! -----------------------------------------------------------------------------------
    ! This module is an alternative to using the whole CH4 module.  The purpose of this
    ! module is to claculate the o2 decomp depth and saturation in unsaturated soils
    ! which is critical for the nitrification/denitrification process.
    ! -----------------------------------------------------------------------------------
...
!    real(r8), intent(out), :: finundated_col                   ! col fractional inundated area (excluding dedicated wetland cols)
!    real(r8), intent(out), :: conc_o2_unsat_col          (:)   ! col O2 conc in each soil layer (mol/m3) (nlevsoi)
!    real(r8), intent(out), :: o2_decomp_depth_unsat_col  (:)   ! col O2 consumption during decomposition in each soil layer (nlevsoi) (mol/m3/s)
